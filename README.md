...
> **Tip:** Add the `uvx code-memory --transport sse` command to your system's startup scripts (e.g. systemd, launchd, or `~/.bashrc`) so the shared server starts automatically.
...
| `--port` | Port for SSE transport | `8765` |
